...
export type ApplicationUserStatus = "active" | "inactive" | "pending" | "rejected";
...
export const createApplicationUser = (applicationId: string, data: ApplicationUserForm) =>
  http
...
export const updateApplicationUser = (
  ...
  http
    .request<unknown>("patch", `/api/applications/${applicationId}/users/${id}`, { data })
    .then(body => unwrapIamPayload<ApplicationUserItem>(body));

export const deleteApplicationUser = (applicationId: string, id: string) =>
  http.request<unknown>("delete", `/api/applications/${applicationId}/users/${id}`);
