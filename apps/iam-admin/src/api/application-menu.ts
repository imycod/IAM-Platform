...
export const createApplicationMenu = (applicationId: string, data: ApplicationMenuForm) =>
  http
...
export const updateApplicationMenu = (
  ...
  http
    .request<unknown>("patch", `/api/applications/${applicationId}/menus/${id}`, { data })
    .then(body => unwrapIamPayload<ApplicationMenuItem>(body));

export const deleteApplicationMenu = (applicationId: string, id: string) =>
  http.request<unknown>("delete", `/api/applications/${applicationId}/menus/${id}`);
