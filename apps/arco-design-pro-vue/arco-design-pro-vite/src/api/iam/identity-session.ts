...
export const getSessions = (params?: {
  ...
  http
    .get<unknown>('/api/sessions', { params })
    .then((body) => unwrapIamPayload<Paginated<SessionItem>>(body));

export const createSession = (data: SessionForm) =>
  http
    .post<unknown>('/api/sessions', { data })
    .then((body) => unwrapIamPayload<SessionItem>(body));

export const updateSession = (id: string, data: Partial<SessionForm>) =>
  http
    .request<unknown>('patch', `/api/sessions/${id}`, { data })
    .then((body) => unwrapIamPayload<SessionItem>(body));

export const deleteSession = (id: string) =>
  http.request<unknown>('delete', `/api/sessions/${id}`);
